...
public class AgentePro : Agent
{
    ...
    [SerializeField] private Transform puertaTransform;           // Posicion de la puerta
    ...
    public override void OnEpisodeBegin()   //resetear posiciones de agente y puerta y colores de placas
    {
        ...
        puertaTransform.localPosition = new Vector3(-0.05f, 1.3f, 6.535f);
        //Nivel 5 descomentar la siguiente linea
        //puertaTransform.localPosition = new Vector3(-5.18f, 7.12f, -6.47f);
        ...
        lastDistanceToGoal = Vector3.Distance(transform.localPosition, puertaTransform.localPosition);
    ...
    {
        //Pasamos como observación el n de placas pisadas normalizado (valores entre 0 y 1 son mejores para la red)
        float placasPisadasNormalizado = (float)placasPisadas / (float)placasMeshRenderers.Count;
        sensor.AddObservation(placasPisadasNormalizado);

        //Pasamos si la puerta esta abierta o no
        sensor.AddObservation(puertaAbierta ? 1.0f : 0.0f);
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        float distanciaActual = Vector3.Distance(transform.localPosition, puertaTransform.localPosition);
        ...
        if (collider.gameObject.CompareTag("Placa"))
        {
            ...
            if (placaMeshRenderer != null)
            {
                if (placaMeshRenderer.material.color != Color.green)  //si no es verde es que no se ha pisado antes
                {
                    ...
                    {
                        puertaAbierta = true;
                        puertaTransform.localPosition += new Vector3(0, 3, 0);  //abrir puerta
                    }
                }
                else
                {
